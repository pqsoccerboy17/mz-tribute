...
- `npm run parse-whatsapp` -- import WhatsApp chat exports to Supabase (uses tsx)
...
**Section order (App.tsx):** Header -> Hero -> MemoryWall -> MediaGallery -> SpotifyEmbed -> AboutMZ -> Footer. Entire app wrapped in `AdminProvider`.

**Data flow:** `useMemories` fetches from Supabase `memories` table with real-time subscription (INSERT/UPDATE/DELETE). Web submissions go through SubmitMemory modal -> `useMediaUpload` (files to storage bucket) -> insert with `source='web'` and `is_approved=false`. Gallery pulls `media_urls` arrays from all approved memories.
